...
addOptional(p, 'summary_type', 1, @isnumeric)
addOptional(p, 'plot_indiv', 1, @isnumeric)
...
plot_indiv          = p.Results.plot_indiv;
...
set(h{1}, 'FaceAlpha', alpha);
    ...
    h{2}.MarkerEdgeColor = zeros(3, 1) + 0.01;

    % info for making mean and SEM trace;
    data_mean = nanmean(X);
    data_sem  = nanstd(X)/sqrt(length(X));

    data_sem  = [data_mean - data_sem, data_mean + data_sem];
...
%             keyboard
            ...
            elseif summary_type == 2 % mean + sem
                % mean line as dot
                h{4} = scatter(data_mean, mean([((-yl(2) * box_dodge_amount) - (wdth * 0.3)) ((-yl(2) * box_dodge_amount) + (wdth * 0.3))]), 50, 'filled', 'MarkerEdgeColor', zeros(3, 1) + 0.01, 'MarkerFaceColor', zeros(3, 1) + 0.01);

                % SEM 
                h{5} = line([data_mean data_sem(2)], [(-yl(2) * box_dodge_amount) (-yl(2) * box_dodge_amount)], 'col', bxcl, 'LineWidth', line_width);
                h{6} = line([data_sem(1) data_mean], [(-yl(2) * box_dodge_amount) (-yl(2) * box_dodge_amount)], 'col', bxcl, 'LineWidth', line_width);        

            elseif summary_type == 3 % mean + confint

                 % mean line as dot
                h{4} = scatter(data_mean, mean([((-yl(2) * box_dodge_amount) - (wdth * 0.3)) ((-yl(2) * box_dodge_amount) + (wdth * 0.3))]), 50, 'filled', 'MarkerEdgeColor', zeros(3, 1) + 0.01, 'MarkerFaceColor', zeros(3, 1) + 0.01);

                % 95% conf int 
                h{5} = line([data_mean data_confint(2)], [(-yl(2) * box_dodge_amount) (-yl(2) * box_dodge_amount)], 'col', bxcl, 'LineWidth', line_width);
                h{6} = line([data_confint(1) data_mean], [(-yl(2) * box_dodge_amount) (-yl(2) * box_dodge_amount)], 'col', bxcl, 'LineWidth', line_width);                
            end
